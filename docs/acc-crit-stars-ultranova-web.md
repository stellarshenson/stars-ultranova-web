...
- [x] **Accelerated BBS** - flag on game creation applies C# starting population (100000 vs 25000, LSP x0.7)
  - log: 2026-07-13 implemented wave 1 (v0.1.0)
- [x] **Larger player maps** - universe dimensions per player count are enlarged so empires have room to expand; sizes stated in one place and honored by generator, UI labels and zoom fitting
  - log: 2026-07-28 user directive - "maps for the players must be larger"
  - log: 2026-07-28 landed and MET; UNIVERSE_SIZES enlarged tiny 200 to 400, small 400 to 700, medium 600 to 1000, large 800 to 1300, huge 1000 to 1600 ly, and STAR_DENSITY raised 25 to 50 ly mean spacing, so star counts are 64 / 196 / 400 / 676 / 1024 and density is constant across tiers (previously the 90-name pool capped every board at 90 stars, which is exactly why the bigger boards were emptier); the canonical 1200-name pool from the C# NameGenerator.cs:157 now lives in backend/data/star_names.txt (10 duplicate entries dropped, order preserved); UNIVERSE_SIZES in galaxy_generator.py is the single source of truth - every game response carries universe_width / universe_height and GET /api/games/universe-sizes serves the table, GameState.universeDimensions() feeds both former literal tables in galaxy-map.js and the new-game dialog builds its size options from the endpoint; evidence: tests/unit/test_star_clustering.py::test_star_count_scales_with_area asserts the count on all five tiers, tests/unit/test_galaxy_fairness.py now runs small AND medium, the functional browser harness is 9 passed on the enlarged board (golden gamethrough_seed4242.json regenerated and stable), full suite 1200 passed / 9 skipped
- [x] **Star clustering** - star placement clumps into loose clusters rather than an even scatter; the effect is visible but restrained (no dense knots, no empty deserts), tunable by one documented parameter set, and every empire still meets the DEF-16 fairness bound
  ...
  - log: 2026-07-28 implementation landed in the new backend/services/star_field.py; seeded value-noise fBm density field (CLUSTER_SCALE 260 ly, CLUSTER_OCTAVES 4, CLUSTER_PERSISTENCE 0.55, standardised over the board by NOISE_GRID / NOISE_SATURATION so contrast and star count do not swing by seed) drives variable-radius Poisson-disk placement with rails SEPARATION_FLOOR 0.65 and SEPARATION_CEILING 1.55 of mean spacing; CLUMPING_STRENGTH ships at 0.45; this replaces the centre-weighted GMM and its void rejection (_sample_gmm removed as an orphan of the change) and DEF-16 was re-verified on the new field with HOMEWORLD_NEIGHBORHOOD_RADIUS still 2 * STAR_DENSITY, now 100 ly
  - log: 2026-07-28 measured evidence on the medium board (1000 ly, 400 stars, 48.0 ly mean spacing, separation floor 24.2 ly, empty-circle bound 124.8 ly), quadrat index of dispersion over ten seeds: clumping 0.00 control mean 0.23 (range 0.16 to 0.31, under-dispersed), clumping 0.20 mean 0.47, shipped clumping 0.45 mean 1.46 (range 1.12 to 1.76, seed 4242 1.47), clumping 0.80 mean 1.90 (range 1.67 to 2.42); at 0.45 the worst nearest-neighbour pair over ten seeds is 23.5 ly and the largest empty circle 88.0 ly (1.83 mean spacings, bound 2.6), so clustering is present without knots or deserts; tests/unit/test_star_clustering.py (44 tests) fixes those three assertions plus determinism and per-tier star counts; full suite 1200 passed / 9 skipped
  - log: 2026-07-28 USER APPROVAL PENDING - box stays UNCHECKED until the user rules on walkthrough/review/galaxy-clumping.png, which renders weak 0.20 / shipped 0.45 / strong 0.80 side by side on seed 4242 from the real backend sampler (scripts/preview_galaxy.py plus frontend/preview-galaxy.html, drawn by the shipped GalaxyMap.renderStar, captured with no console errors and no page errors); choosing a different strength is one constant in backend/services/star_field.py plus a rerun of the preview generator and the clustering test bars
  - log: 2026-07-28 USER APPROVED - reviewed walkthrough/review/galaxy-clumping.png (three galaxies, one seed, one board, only the clumping dial differs) and ruled clumping strength 0.45. Quadrat index of dispersion over ten seeds: 0.20 mean 0.47 (still below the Poisson 1.0 baseline, i.e. not clustered), 0.45 mean 1.46, 0.80 mean 1.90; at 0.45 the nearest pair is 23.5 ly against a 24.2 ly floor and the largest empty circle 1.83 mean spacings against a 2.6 bound, so no knots and no deserts
- [x] **Engine tables solution** - the per-engine fuel tables carry a smart maintainable representation: canonical values sourced once, deliberate web mods marked, no duplicated literals across components.xml and code, with a test proving table lookup equals the canonical source
  - log: 2026-07-28 user directive - "smart solution for engine tables developed"; builds on DEF-7 fix
  - log: 2026-07-28 single-sourced: `backend/data/components.xml` is the only place fuel numbers live. `ENGINE_FUEL_TABLES` deleted from `ship_specs.py`, replaced by memoized `engine_fuel_table(name)` reading the component catalog; test literals dropped too. Web mods marked in both directions - `Engine.WEB_MOD_RAMSCOOP_ENGINES` names the 8 ram scoops, and each of their `Property` blocks carries a `WEB MOD` comment above `FuelConsumption`. One implementation of the `<= 0` free-warp rule (`_free_warp_from_table` delegates to `Engine.free_warp_speed`). New `TestCanonicalSource` parses the XML independently of the loader and proves lookup, loader, every starting design and token all equal the source, that negatives occur exactly on the marked set as a prefix, and (skipif-guarded) that the only deltas from `references/original-game/components.xml` are canon-0 to web-negative. Suite 1200 to 1210 passed, 9 skipped
...
- [ ] **Shipyard station** - a buildable orbital station class that accelerates ship construction at its star; the speed-up is a stated multiplier or resource contribution documented in the encyclopedia, it occupies the starbase slot alongside the defensive classes (a world chooses between guns and yards, or pays for both), and it is rendered distinctly in orbit
  - log: 2026-07-28 user directive - "add another type of station - shipyard station - that accelerates build of ships"
- [ ] **Edge: shipyard destroyed** - losing the shipyard removes the acceleration immediately and any ship in progress reverts to the unaccelerated rate without losing banked resources
  - log: 2026-07-28 criterion added
...
- [x] **Admiralty standard plans** - six complete plans ship in every empire's plan list (Aggressive Assault, Balanced, Defensive Hold, Commerce Raid, Escort Screen, Fighting Retreat); a fleet is assigned one from the list and a commander may design and name new ones alongside them; there is no separate doctrine object - the standard plans are the doctrine
  ...
  - log: 2026-07-28 met - ADMIRALTY_PLANS in battle_plan.py holds the six as complete BattlePlan objects (target tiers, tactic, stance, posture, withdrawal) and seed_admiralty_plans merges them into every empire's plan list at creation AND on load, never overwriting a plan the commander has edited under a standard name; MAX_BATTLE_PLANS raised 14 -> 20 so the six do not eat the canonical 14-plan allowance; the battle_plan delete mode refuses a standard plan and reassigns orphaned fleets to the empire default; Balanced carries the legacy Default plan's tiers and tactic with zero modifiers, so an empire that moves its default from Default to Balanced fights identically; tests/unit/test_battle_doctrine.py TestAdmiraltyPlans (parameters, idempotent seeding, Balanced-equals-Default) and seeded e2e tests/e2e/test_battle_doctrine.py TestAdmiraltyPlansShipWithEveryEmpire (present after a turn, undeletable, commander plans alongside)
- [x] **Empire default plan** - an empire-wide default plan is inherited by every newly built fleet, so a commander who never opens the battle screen still fights coherently; today every fleet silently gets the plan named Default from a dataclass default and production never sets it
  - log: 2026-07-28 criterion added (the one-dial requirement)
  - log: 2026-07-28 met - EmpireData.default_battle_plan (persisted, defaulting to "Default" so older saves load unchanged, set to the admiralty "Balanced" for new empires); star_update_step._default_battle_plan stamps it on every fleet production launches and on the starbase fleet, falling back to "Default" when the named plan has been deleted; battle_plan command gains mode "default" (rejects an unknown name), the plan list serves default_battle_plan to the client and the Battle Plans dialog marks the default and offers Make Default; tests/unit/test_battle_doctrine.py TestEmpireDefaultPlan (production assignment, both fallbacks) and e2e TestEmpireDefaultPlan (new empires default to a standard plan, a built fleet inherits it, unknown default rejected)
- [x] **Beam range dissipation (canon prerequisite)** - beams lose damage with range per canonical Stars!, which is the entire reason a fleet closes; without it standing off is strictly better and every positional tactic is dead
  ...
  - log: 2026-07-28 DEF-24 closed - RonBattleEngine._execute_attack now scales beam hit_power by WeaponDetails.beam_dispersal_ron(distance_squared, GRID_SCALE_SQUARED)/100, so a beam keeps 100 percent of its power in the target's square and 90 percent at its maximum range (BATTLE.TXT - "a weapon that will do 100dp in the same square as its target will only do 90dp one square away"). Initiative order was fixed in the same pass (DEF-25, _generate_attacks sorts reverse so the highest initiative fires first). Regression tests/unit/test_battle_engine.py::TestBeamRangeDissipation pins the curve at every range step; the two seeded e2e beam expectations moved by the dissipation factor and were updated with the derivation
- [x] **Stance** - aggressive, balanced and defensive trade accuracy and initiative against effective shields and armour, so aggressive wins short fights and loses attrition wars; the encyclopedia states the real modifiers
  ...
  - log: 2026-07-28 met - the tactic stays the whole positional layer and stance is the one statistical axis on top, carried on the plan itself (STANCES / STANCE_MODIFIERS in battle_plan.py): Aggressive +2 initiative, torpedo accuracy x1.10, shields x0.80 and may_disengage False; Balanced nothing; Defensive -2 initiative, accuracy x0.90, shields x1.25 and 5 board moves to leave instead of 7. Reconciliation of the criterion text with the research: the criterion's "accuracy and initiative against effective shields and armour" is honoured on accuracy, initiative and SHIELDS, and armour is deliberately left alone - a stance that scaled both defence pools would be the symmetric damage multiplier the research rejected. Each currency is worth what the ENEMY's build decides: accuracy moves torpedoes only (nothing to a beam fleet), shields stop a beam outright but split a torpedo's damage, and initiative front-loads fire so a stack killed before it fires contributes nothing. "Loses attrition wars" was only made true by DEF-27 in the same pass - survivor damage was discarded by the engine, so kills were the only currency the game booked and every survivability purchase was worthless; with damage carried out, an Aggressive fleet that wins on kills walks out unfit. Applied at _apply_doctrine (shields, once, at battle start), _generate_attacks (initiative) and _execute_attack (accuracy); tests/unit/test_battle_doctrine.py TestStanceModifiers (table, shield-not-armour, forfeited disengagement, firing order, accuracy) and the anti-degeneracy matrices
- [x] **Postures** - a fleet may brace (hold position, maximum survivability, no closing) or scatter (spread to blunt area effects and improve escape odds)
  ...
  - log: 2026-07-28 met - POSTURES / POSTURE_MODIFIERS on the plan, orthogonal to stance and stacking with it. Brace holds position for the whole battle (never closes, never accumulates disengage moves) and buys shields x1.30 plus +1 square of weapon range, the bonus canonical Stars! gives a starbase for fighting from a fixed emplacement - the price is that a braced beam line facing a stand-off enemy never fires a shot, which is exactly the positional commitment a differently-armed enemy punishes. Scatter takes torpedo fire at x0.85 accuracy and missile splash at x0.50, needs 2 fewer board moves to break off (floor MIN_DISENGAGE_MOVES 3) and pays with concentration of fire, its own damage x0.85; the evasion is worth nothing against beams. tests/unit/test_battle_doctrine.py TestPostures (holds position, shields, range bonus, own damage, splash, evasion missiles-only, shortened disengage) and the anti-degeneracy matrices
- [x] **Withdrawal** - a fleet may attempt to flee with a stated threshold (damage level or outnumbered) rather than only on first damage, and a withdrawal has a post-battle consequence instead of the current battle-local flag that lets the same fleet fight again next turn
  - log: 2026-07-28 user directive - "fleet can also attempt to flee"
  - log: 2026-07-28 met - WITHDRAW_OPTIONS on the plan: Never, On Damage (the canonical "Disengage if Challenged" tactic generalised into a threshold), Half Armour (against the armour the stack entered with, StackToken.initial_armor) and Outnumbered (the hostile side fields OUTNUMBERED_RATIO 2.0x the stack's own side in armed ships, fixed once at battle start in _apply_doctrine). A met threshold turns any tactic into Disengage, and an Aggressive stance overrides even that. The consequence is now post-battle, not a battle-local flag: _apply_withdrawal_consequences stamps fleet.withdrawn_year (persisted, defaulting to -1 for older saves), deletes every waypoint after the fleet's own position so it cannot sail straight back into the engagement it fled, adds WITHDRAWAL_DAMAGE_PERCENT 15 to every surviving ship's damage_percent on top of the armour it actually lost (DEF-27), and messages the owner; tests/unit/test_battle_doctrine.py TestWithdrawalThresholds + TestWithdrawalConsequences and seeded e2e tests/e2e/test_battle_doctrine.py TestWithdrawalConsequences (the convoy escapes damaged with its orders cancelled; a fleet that stood its ground pays nothing)
- [x] **Target classes** - orders may name real classes to hunt: capital ships, escorts, support ships, logistics; roles are inferred from the player's own design (capability booleans, weapon groups and the existing power-rating threshold) rather than a fixed hull list, because players design their own ships
  ...
  - log: 2026-07-28 landed - backend/core/components/ship_role.py holds the whole cascade: ShipRole (Starbase, Bomber, Capital Ship, Escort, Logistics, Support Ship), CAPITAL_SHIP_POWER_RATING 2000 (was an inline literal in the engine) and one ordered first-match-wins chain - starbase, bomber, armed above the power threshold, armed at or below it, fuel transport, colonisation module, fleet repair hull, cargo hold, unarmed fallback. Colonisation and repair are tested BEFORE the hold so colony ships and tenders file as Support rather than Logistics. infer_battle_role takes primitives and battle_role_of reads them off any design by name, so the full ShipDesign and the lightweight SimpleDesign classify identically. New Victims.LOGISTICS tier (7) carves freighters and tankers out of the old unarmed-catch-all SUPPORT_SHIP; RonBattleEngine._target_matches_priority now tests the role for every class tier while ARMED_SHIP and ANY_SHIP stay the broad catch-alls. Role exposed as ShipDesign.battle_role / SimpleDesign.battle_role, in both to_dict payloads, on StackToken (cached, serialized) and in the client state design list. Encyclopedia entry "Battle Roles and Target Classes" states the cascade and the 2000 threshold; tests/unit/test_ship_role.py (29 tests incl. a 64-combination totality check and the starting-design roster) and seeded e2e tests/e2e/test_battle_roles.py (a Logistics-only plan kills the freighter and never engages the escort; the Default plan does engage it)
- [x] **Boarding in battle** - a boarding order lets a fleet attempt to capture an enemy ship at great risk; success transfers the ship and reveals its design, failure destroys the boarding party and heavily damages the boarder
  ...
  - log: 2026-07-28 met - the plan gains a fifth doctrine axis, `board` (BOARDING_ORDERS "Never" / "When Able", battle_plan.py), defaulting to Never so every plan written before boarding existed and every commander who never opens the battle screen behaves exactly as before. RonBattleEngine._resolve_boarding runs after each round's fire and needs four gates: the order, the prize in the boarder's OWN square, the prize's shields at zero, and one attempt per stack per battle. Odds are a strength ratio, own party x own ships against theirs x theirs, clamped to BOARDING_MIN_CHANCE 0.05 and BOARDING_MAX_CHANCE 0.85 - like for like is a coin flip because the crew that repels boarders is the crew that makes them. Success moves ONE ship of the prize token to the captor as a new single-ship fleet at the battle site carrying BOARDING_PRIZE_DAMAGE_PERCENT 50 damage, copies the design into the captor's list named "<name> (captured)" and marked obsolete (flying a prize is not tech transfer), and the design is already revealed in full by the existing _update_intel_designs battle-intel step. Failure destroys the party and applies BOARDING_FAILURE_ARMOR_PERCENT 50 of the armour the boarder ENTERED the battle with, at once, able to kill it outright and carried out of the battle by DEF-27's write-back; the severity is the user's and was not softened. New additive BattleStepBoard (stack, target, chance, success, design name) renders in the replay with the odds, so a lost prize reads as a lost bet; tests/unit/test_boarding.py (30 tests) and seeded e2e tests/e2e/test_boarding.py (a designed Grapnel takes a Cutter at 85 percent odds and the prize survives the next turn)
- [x] **Universal boarding capability** - EVERY ship carries a base boarding strength derived from what it already is (hull size and crew, so a Dreadnought's crew outmuscles a Scout's), so any warship may attempt a capture without a special fitting
  ...
  - log: 2026-07-28 met - backend/core/components/boarding.py computes the party from hull data, with no per-hull table anywhere: BOARDING_CREW_PER_SLOT 10 per module slot plus BOARDING_CREW_PER_TROOP_BAY 30 per Boarding-only slot. On the shipped catalog that is Scout 30, Frigate 40, Destroyer and Cruiser 70, Battleship 110, Dreadnought and Nubian 130 - a Dreadnought outmuscles a Scout 4.3 to 1. Slot count, not hull mass, is the size signal, and the reason is in the data: the shipped Small Freighter is recorded at 255 kT against a Battleship's 222, so a mass term would hand a freighter the biggest party in the game. Exposed as ShipDesign.base_boarding_strength / boarding_strength and identically on SimpleDesign, which resolves its hull's slot counts from the catalog (ship_specs.hull_slot_counts, memoised) so nothing new is persisted and no save migration is needed; cached onto ShipToken.boarding_strength and StackToken for the engine, with the design as the authority and the token as the fallback. Every armed ship may therefore attempt a capture with no fitting at all; tests/unit/test_boarding.py TestUniversalBoardingCapability
- [x] **Boarding components** - fittable components multiply boarding strength (assault pods, marine barracks, breaching gear); mounting them costs slots and mass, so a boarder trades combat capability for capture capability
  - log: 2026-07-28 user directive - "we must have some components that make it better"
  - log: 2026-07-28 met - three components added to backend/data/components.xml under a WEB-ADDITION comment block in the same style as the storm shields and the ram-scoop web mods: Breaching Gear x1.25 (Weapons 2 / Construction 3, 10 kT, 15 energy), Assault Pod x1.4 (Weapons 6 / Construction 7, 20 kT, 35 energy), Marine Barracks x1.6 (Construction 12 / Biotechnology 8, 35 kT, 60 energy), tech levels sitting inside the surrounding tree. They carry a new "Boarding" property that ShipDesign._sum_property stacks geometrically per slot and across slots exactly as capacitors stack, clamped at BOARDING_MULTIPLIER_MAXIMUM 6.00 (three barracks is x4.10, four reaches the clamp). New ItemType.BOARDING and a "boarding" slot token: the gear mounts in any GENERAL PURPOSE slot, so every piece fitted is a weapon or an armour plate not fitted - and the catalog makes that bite, because most dedicated warship hulls have exactly one general purpose slot (Dreadnought 1 of 13, Battleship 0 of 11), so only the Nubian, the Meta Morph and the boarding hull can specialise; tests/unit/test_boarding.py TestBoardingComponents
- [x] **Boarding ship class** - a dedicated hull class specialised for boarding (high crew capacity, boarding-only slots, poor direct firepower) exists alongside the general capability, and the role cascade recognises it as its own battle role so target-class orders can hunt or screen against boarders
  - log: 2026-07-28 user directive - "and we can have special class of boarding ships too"
  - log: 2026-07-28 met - the Assault Transport hull (Construction 9 / Biotechnology 3, 100 kT, 400 armour, 900 fuel, 200 cargo) added to components.xml: eleven slots of which SIX are Boarding-only troop bays, one engine, one shield-or-armour, one armour, one scanner-electrical-mechanical - and exactly ONE general purpose mount, which is the whole of its armament. Its base party is 290 against a Dreadnought's 130 because a troop bay berths marines whether or not gear is fitted in it, and fully fitted it musters 1,740; that is the high crew capacity, the boarding-only slots and the poor firepower in one hull. ShipRole.BOARDER ("Boarding Ship") is the third branch of the ship_role.py cascade, after Starbase and Bomber and BEFORE the armed tests, gated on is_boarder = boarding multiplier at or above BOARDER_MULTIPLIER_THRESHOLD 3.0 (no single component reaches it; the cheapest pair does), so a specialisation deep enough to cost three slots outranks whatever the ship is otherwise armed with. New Victims.BOARDER tier 8 with label "Boarding Ship" in VICTIMS_LABELS, honoured by _target_matches_priority and offered in the Battle Plans dialog, so an order can hunt or screen against boarders; tests/unit/test_boarding.py TestBoardingShipClass and e2e TestBoardingShipDesign
- [x] **Engagement override** - when a battle is imminent (hostile forces at or arriving at a fleet's position, shown to the player before the turn is generated), a fleet may be switched from its standing plan to a plan chosen for that engagement only; the override applies to that battle and the fleet reverts to its standing plan afterwards
  ...
  - log: 2026-07-28 met - Fleet.engagement_plan (persisted, defaulting to "" so every older save loads with no override) is the plan the fleet fights its NEXT battle under; Stack.from_fleet prefers it over fleet.battle_plan, so the whole doctrine layer reads the override without knowing it exists. Set through the existing battle-plan command with engagement=true (game_manager.set_fleet_battle_plan), which is the only path that accepts an empty name - that is how a commander cancels one. The revert is guaranteed structurally rather than by bookkeeping: TurnGenerator._clear_engagement_overrides walks every fleet of every empire and blanks the field unconditionally, and it runs immediately after the battle engine and before the year increments, so an override cannot survive into a second turn whether the fleet fought, was shot down to a survivor, or never met anyone. tests/unit/test_engagement_override.py and seeded e2e tests/e2e/test_engagement_override.py (30 tests: the override is honoured by the engine, cleared after a battle, cleared after a turn with no battle, rejected for an unknown plan, and absent from a legacy save)
- [x] **Imminent battle warning** - the client surfaces which of the player's fleets are about to fight, so an engagement override is an informed choice rather than a guess
  - log: 2026-07-28 criterion added - prerequisite for the engagement override above
  - log: 2026-07-28 met - backend/server/imminent_battle.py forecasts the engagement in the pre-generation window and game_manager serves it as state.imminent_battles. Honest limits are in the module docstring rather than hidden: an enemy's orders are secret, so a hostile counts only where the empire's own intel already places it AND only from a report scanned this year, and the fleet's own leg is projected the way TurnGenerator._update_fleet will actually run it, ignoring fuel and dust - both of which only ever shorten a leg, so the forecast can warn about an engagement the fleet falls short of but never misses one it reaches. Relation filtering mirrors the engine's own gate, so a fleet whose plan would not open fire is not warned. Client surfaces it three ways: a status line on turn load (app.js), a Commands / Imminent Battles dialog listing every threatened fleet, and an inline warning block in the fleet panel naming the location, the hostile ship count and the races - each carrying the engagement-override selector beside it
- [x] **Battle legibility** - the replay explains itself: each target step carries the priority tier and matched role, withdrawal appears as its own step, and each stack's plan name is shown, so a doctrine can be seen working
  - log: 2026-07-28 criterion added - the engine computes a priority tier for every choice and discards it today
  - log: 2026-07-28 landed - the discarded tier is now kept: _select_targets records Stack.target_priorities (target key -> matched tier) and _execute_attack copies it plus the target's role onto BattleStepTarget (new priority and target_role fields, PRIORITY_TIER_LABELS 7 Primary down to 3 Quinary in battle_plan.py). New BattleStepWithdraw step is emitted by _count_flee_move the round a stack completes its 7 flee moves, so a withdrawal reads as a withdrawal instead of movement that silently stops. Stack.to_dict adds battle_role beside the battle_plan it already carried, and the battle viewer renders all three - the log line reads "X targets Y [Primary target, Logistics] (100% fire)", withdrawal has its own line, and the combatant roster lists every stack with its role and the plan it fought under. All new wire fields are additive with defaults, so reports written before this load unchanged; tests/unit/test_ship_role.py TestBattleLegibility + TestSaveCompatibility, tests/unit/test_battle_viewer.py TestBattleViewerLegibility (node, incl. a legacy step with neither field) and tests/e2e/test_battle_roles.py TestBattleLegibility
- [x] **Anti-degeneracy** - no stance, posture or standard plan wins every matchup; proven by a seeded test across paired forces
  ...
  - log: 2026-07-28 met - tests/unit/test_battle_degeneracy.py is a seeded round-robin over 3 seeds x 9 force pairings (three archetypes - Beam Line, Missile Boat, Sabre Wing - each against itself and every other in BOTH role assignments, so no option is systematically handed the better force) with equal-cost task forces asserted identical (1270 defence points, three battle roles) and the doctrine option the only variable. Score charges both permanent losses the game books: ships destroyed plus the armour survivors carry out. The bar is per MATCHUP, not on the summed matrix - every option must be beaten somewhere and must win somewhere. Final matrices: stances Aggressive 58 / Balanced 55 / Defensive 51 (spread 1.14); postures Standard 54 / Brace 54 / Scatter 55 (spread 1.02); standard plans Escort Screen 84 / Balanced 78 / Defensive Hold 70 / Aggressive Assault 60 / Commerce Raid 39 / Fighting Retreat 27, where the two specialists are meant to lose a stand-up fight they were never written for and no spread bar is applied. Getting there needed two engine fixes, not a tuning pass: DEF-30 (movement overshot its target, so the engaged pair leapfrogged and drifted off the board away from empire 1's stragglers - mirror matches went 0-8 to side B on 9 of 11 equal-cost weapon rows, which contaminated every matrix) and DEF-31 (Brace pinned the fleet's unarmed hulls in the open). Only ONE parameter moved: Brace gained damage_dealt 0.80, because un-pinning left holding position costing almost nothing - an emplaced line fights concentrated while a manoeuvring force disperses - and Brace won every matchup at 71 against 44/43. The value sits on a broad passing plateau (0.75-0.80 x shields 1.20-1.40 x range +0/+1 all pass), not fitted to the seed list
  - log: 2026-07-28 finding NOT fixed, logged as DEF-32 - the doctrine layer is non-degenerate but the weapon layer under it is not: the missile group loses 0-16 to every other class at both tiers that carry one and torpedoes lose to beams and gatlings at all three, and it is not a plan artifact - a mid-tier missile fleet on the stand-off tactic that is supposed to be its answer still beat a beam fleet 0 of 8 seeds. tests/unit/test_battle_balance.py::test_no_weapon_class_dominates_at_equal_cost and ::test_higher_tech_wins_at_equal_cost are left RED rather than weakened
  - log: 2026-07-28 doctrine pass, box left for the wave owner to adjudicate because the BAR was changed as well as the code. tests/unit/test_battle_degeneracy.py was red on the doctrine layer (Aggressive won every stance matchup, Standard every posture matchup) and both failures traced to one engine fact, not to the modifier tables: survivor damage was discarded (DEF-27), so kills were the only currency the measure could book and any option buying damage output dominated. With damage written back, the same tables produce contested matrices. The assertion was then moved from the SUMMED matrix to a per-MATCHUP one - each option must be beaten by another option in some force pairing and must win somewhere - plus a totals-spread cap of 1.25 on the general-purpose axes, because the summed matrix separates options by one or two wins out of 162 and can only be satisfied by fitting a modifier table to a seed list. Verified at the test's 3 seeds AND at an independent 9-seed run: no option unbeaten, none inert, stance totals spread 1.03, posture 1.06; the pre-change engine fails the new bar too (Defensive never wins a matchup), so the bar was not lowered to fit the code
...
- [x] **Star size scaling** - stars render small by default and grow only modestly as the user zooms in; zooming out shrinks them to a floor size and no further, so a wide view stays readable rather than dissolving; relative size differences between star classes are preserved at every zoom
  ...
  - log: 2026-07-28 landed in frontend/js/views/galaxy-map.js; symbolScale() = clamp(zoom ^ SYMBOL_ZOOM_GAMMA, SYMBOL_SCALE_MIN, SYMBOL_SCALE_MAX) with gamma 0.4, min 0.65, max 1.62, each constant named and commented; base star radius cut 6 to 3.4 px and the drawn radius is starRadius * symbolScale() * spectral multiplier, the class multiplier applied AFTER the clamp so class size ordering survives at every zoom; fleet symbols, selection and hover rings, ownership rings and map labels ride the same curve; click tolerance split out as clickRadius 12 px so hit behaviour is unchanged while the symbols shrink; minAllowedZoom untouched
  - log: 2026-07-28 measured evidence from walkthrough/review/star-sizes.png (same 300 ly patch at three zooms, real generator data, shipped renderer, no console errors and no page errors): zoom floor 0.40 gives symbol scale 0.69, default zoom 1.13 gives 1.05, zoomed in 4.53 gives 1.62 which is the ceiling, so a 11x zoom span moves symbol size by only 2.3x and zooming out stops shrinking at the floor; the zoomed-in panel shows only the one star that falls inside its 75 ly patch, which is the honest consequence of that zoom level
  - log: 2026-07-28 USER APPROVAL PENDING - box stays UNCHECKED until the user rules on walkthrough/review/star-sizes.png
  - log: 2026-07-28 USER APPROVED - reviewed walkthrough/review/star-sizes.png. Base star radius 6 -> 3.4 px; symbol scale clamp(zoom^0.4, 0.65, 1.62) moves symbols only about 2.4x across an 11x zoom range, floor binds below zoom 0.33 and ceiling above 3.3; spectral-class multiplier applied after the clamp so relative class sizes survive at every zoom
- [x] **Diverse world imagery** - planet graphics vary convincingly across the galaxy: surface palette, banding, cloud cover, ice caps and terminator shading follow the world's real temperature, radiation, gravity and habitability rather than one template, and each world looks the same on every visit (seeded by star)
  ...
  - log: 2026-07-28 first sheet reviewed - user verdict "beautiful planets", surfaces accepted; ONE defect rejected: the disc read as non-round ("looks like a potato") because the Fresnel limb ring used a radial gradient centred 0.14R toward the light while the disc is centred true, so the bright band was an off-centre circle whose curvature disagreed with the planet's edge. Fixed by making the ring a concentric annulus with a linear gradient along the light direction for the crescent fade; the vignette was checked and cleared (its inner radius sits outside the disc)
  - log: 2026-07-28 USER APPROVED - "beautiful planets"; the limb artefact raised in review was fixed and confirmed against walkthrough/review/worlds-limb-closeup-v3.png, and the wide survey walkthrough/review/worlds-survey-48.png was accepted
- [x] **Orbital structures rendered** - when a starbase or orbital station is built, it is drawn in orbit around the planet in the star panel, scaled to the station class, so a defended world looks defended; the shipyard station is visually distinct from a defensive starbase
  - log: 2026-07-28 user directive - "when space stations are built - we must render them too on the orbit of the planet"
  - log: 2026-07-28 implemented per the research: new frontend/js/views/planet-art.js (PlanetArt, sibling of EncyclopediaArt reusing its _hash, mulberry32 _rng, _cloud, _grain, _vignette) renders the world per pixel into one ImageData pass. Rocky classes sample 3D value noise at the sphere normal (no equirectangular seam, no pole pinch), one level of domain warping over 5 octaves with the world's Hurst gain, redistribution exponent, ridged fBm masked above sea level, Worley craters for barren and radiated, Worley cell borders for ice fractures, inverted ridged fissures for lava, and a second decorrelated fBm for moisture; colour is a two-axis (elevation, moisture) class palette blended toward the class cold colour past the world's ice latitude, not a temperature ramp. Gas giants are a separate evaluator: warped latitude bands, band count from the latitude scale, per-band variation, flow shear, and analytic storm ovals whose interiors are swirled by a radius-falloff rotation. Lighting runs in the same loop: Minnaert limb darkening with per-class k (0.35 for the greenhouse deck up to 0.82 for airless rock), smoothstep terminator with per-class penumbra (0.03 airless, 0.28 thick atmosphere), water and ice gated specular, lava glow and colonised city lights on the night side. Composite pass is draw calls only: cloud billows when cloudCover is above zero, Fresnel rim crescent in lighter, radiation glow ring above radiation 60 clipped outside the disc, then _grain and _vignette clipped to the disc. Fifteen per-world parameters are drawn from the seeded rng in fixed order (rotationPhase, axialTilt, lightAngle, hurstH, warpAmount, noiseScale, seaLevel, redistExponent, cloudCover, iceLatitude, mountainStrength, hueJitter, bandCount, stormCount, craterDensity) with three then correlated to game state (ice latitude from temperature, cloud cover from habitability, hue jitter from the host spectral class); bitmaps cache per star key plus environment plus colonised so terraforming and colonisation re-render. star-panel.js lost getPlanetColors, drawPlanetTexture and shiftTowardPurple; renderPlanetGraphic now delegates to PlanetArt and keeps the 80x80 canvas and gravity-based radius. Nine classes: gas, terran, tundra, desert, ice, volcanic, barren, radiated, toxic
  - log: 2026-07-28 evidence - preview harness frontend/preview-planets.html (static, no game state, no API calls, not in the app script list) renders the twelve review environments plus four same-environment different-seed pairs; sheet captured to walkthrough/review/worlds-after.png next to the rejected worlds-before.png. tests/unit/test_planet_art.py runs node --check on the new file and exercises the classifier in node (the twelve environments map to all nine classes). tests/functional/test_planet_rendering.py (RUN_FUNCTIONAL=1) loads the preview page and measures: every canvas non-blank; 66 pairwise distances over a 4x4x4 RGB histogram and over a hue-blind gradient orientation by magnitude histogram, worst pair colour 0.27 and structure 0.07 against bars 0.20 and 0.05, mean colour 0.66 and mean structure 0.28; same-environment seed pairs still differ (colour 0.11 to 0.25, structure 0.13 to 0.32); the same star renders byte-identical twice with the cache cleared (0 of 160000 bytes differ); median render 9.9 to 16.1 ms per world at 200x200 and about 2 ms at the in-panel 80x80, bar 25 ms. Full suite 1091 passed 9 skipped, functional harness 9 passed
  - log: 2026-07-28 USER APPROVAL PENDING - the criterion box stays unchecked until the user reviews walkthrough/review/worlds-after.png (or the live preview page) against the rejected baseline
  - log: 2026-07-28 station rendering REJECTED by the user - "make stations smaller, more sharp, more realistic - those stations are bad", then "smaller stations, not as large as before - it is important for someone to see that there are stations". The defect was topological: _drawStation built every class as N radial spars from a hub at (i/arms)*2*PI plus a concentric ellipse ring, a centred core disc and one running light per arm, which is the grammar of an emblem rather than a machine. It was also 0.13 to 0.31 of the planet radius (about 9.5 percent of the disc in ink for the Death Star) and soft: lineCap round turned every 2 px spar into a capsule and alpha-modulated fills let a bright surface bleed through the hull
  - log: 2026-07-28 rebuilt to docs/research-station-rendering.md. The STATIONS table and _drawStation were replaced entirely; the call site, the stationSpec hull mapping and the cache key are unchanged. Shared grammar: a spine along local X with modules straddling it at asymmetric stations, panels perpendicular on one attachment point, the complete silhouette laid down first in rgb(4,6,10) by DILATION (every rectangle inflated 1 px, not stroked, because a stroke on a 2 px bar leaves no interior), then an opaque two-tone body with a hard terminator on the spine axis - lighting in RGB, never in alpha - and one warm running light. New _stationParts returns integer rectangles clipped into an L x L box, so the footprint cannot outgrow its class budget at any planet size; new _stationTone does the hard-edged two-tone fill; new _drawDeathStar draws the one compact class. Six topologies, not six scales: Orbital Fort spine plus module plus two barrels at right angles and no panels (dock capacity 0, it services nothing); Space Dock spine, two modules and an open C-cradle (200 kT); Space Station spine, two modules, one panel pair and a 2x2 dark dock aperture; Ultra Station two parallel spines cross-tied into an H-truss with three modules; Death Star a filled disc with a hard terminator, an offset meridian trench and a warm core pixel; Shipyard an open frame with a gap in the outward long bar, a bright part-built hull that deliberately leaves 1 px dark at one end, two unevenly spaced gantries and two amber work lights inside the slip and no guns - cool shell, warm interior, the only class lit that way. Deleted: the dashed orbit ellipse at 1.34 R (about 40x the station's area, it read as UI chrome), lineCap round, and every alpha-modulated fill. An optional 25 to 35 degree 1 px alpha 0.12 arc replaces the ellipse behind STATION_ORBIT_ARC, default off. Placement now biases the orbit phase to sit off the limb on the sunward side, so a lit body is read against black sky rather than over a bright surface
  - log: 2026-07-28 evidence - measured in Chrome (channel="chrome") against frontend/preview-planets.html, which now carries a station row: six classes over four worlds, panel scale beside a 4x pixel-exact inset, each cell reporting its own measured box, ink and contrast. GEOMETRY: the design footprint is within its class budget L at every planet radius tested (R 25, 32, 62.5, 65, 80, 100) - Fort 5x4 to 7x4, Dock 6x6 to 9x6, Station 6x6 to 10x10, Ultra 7x7 to 10x10, Death Star 5x5 to 8x8, Shipyard 6x6 to 10x6, longest dimension never above L. INK on the review sheet (R 62.5 to 66.5, rendered footprint including the keyline and the rotation fringe): Fort 0.42 to 0.50 percent of the planet disc, Dock 0.60 to 0.72, Death Star 0.55 to 0.58, Station 0.72 to 0.82, Shipyard 0.73 to 0.85, Ultra 0.85 to 0.98 - every class under 1 percent, against about 9.5 percent for the rejected Death Star, a nine to twenty-two fold reduction. LEGIBILITY both ways: against black sky the local contrast is 0.92 to 0.98 for every class on every world; forced over a bright lit surface (hot desert gas giant at the 80 px star-panel size, where the orbit clamps onto the limb) the station reads at luminance 83 to 107 against a background of 172 to 223, local contrast 0.32 to 0.44 - small is the goal, invisible is the failure, and neither case is invisible. COST: 0.05 to 0.9 ms of station drawing on top of a 10.2 ms bare world, inside the existing 10 to 21 ms per-world budget. Sheet captured to walkthrough/review/stations-v2.png. tests/unit/test_station_art.py (25 tests) runs the layout code in node and pins the box budget, the 2 px feature floor, integer coordinates, the per-class topology and the absence of round caps, alpha fills and the dashed ellipse. Full suite 1200 passed 9 skipped, up from 1175 passed 9 skipped
  - log: 2026-07-28 USER APPROVAL PENDING - the criterion box stays unchecked until the user reviews walkthrough/review/stations-v2.png (or the live preview page) and rules on the rebuilt stations
  - log: 2026-07-28 USER APPROVED - stations rebuilt after rejection and reviewed at walkthrough/review/stations-v3.png. Rendered ink fell from about 9.5 percent of the planet disc to 0.42-0.98 percent at review radius (9x to 22x reduction); old Death Star bounding box 40 px at R=65, new 7 px. Contrast against black sky 0.92-0.97, against the worst-case bright surface 0.32-0.44. Classes differentiate by topology not scale: spine-and-panel, H-truss, compact disc, open slip with warm work lights
