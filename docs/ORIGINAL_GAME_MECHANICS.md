...
- Orders/turn-submission model: per-player orders files, turn-submitted flags, password per race [C# ok] →web: **missing** - user requirement 2026-07-13: correspondence play (turn package / orders file / full-game handoff, passwords, turn locking) - criteria in acc-crit Correspondence Play section; live simultaneous multiplayer stays out of scope

## 17. Absent in the C# reference (canonical Stars! - implement from rules if wanted)

- Wormholes (done in web), mineral packets and mass drivers (web has a `_move_mineral_packets` remnant - flesh out or cut), mystery trader (only a TODO comment; user directive 2026-07-13: build from canonical rules as a first-class feature - criteria in acc-crit Mystery Trader section, scheduled wave 5), random events (comet strikes etc.), tachyon detection (done in web - counters cloak on the scanning fleet), mine sweeping (done in web - beam power × range² yearly, gatling as range 16, plus SD detonation), pop transfers between own worlds via waypoints (done in web - waypoint CargoTask colonist amounts), diplomacy beyond 3-state relations
